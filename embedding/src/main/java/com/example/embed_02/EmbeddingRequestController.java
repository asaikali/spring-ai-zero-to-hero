...
public class EmbeddingRequestController {
    ...
    public EmbeddingRequestController(EmbeddingClient embeddingClient) throws IOException {
        ...
        this.shakespeareWorks = new ClassPathResource("/data/books/Shakespeare.txt").getContentAsString(StandardCharsets.UTF_8);
    }
    ...
    public String bigFile()  {
        try {
            ...
            return "Success document of length %s characters, was emmbeded into 1 vector wit dimension %s".formatted(shakespeareWorks.length(),embedding.size());
        } catch (Exception e) {
            return """
    ...
    public  String chunkFile() {
        ...
        return "file split into %s chunks %s embeddings created because we don't want to waste money by embedding every chunk".formatted(chunks.size(),embeddings.size()) ;
    }
