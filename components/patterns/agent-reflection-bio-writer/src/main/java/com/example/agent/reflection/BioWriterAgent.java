...
@Service
public class BioWriterAgent {

  private ChatClient chatClient;

  public BioWriterAgent( ChatClient.Builder chatClientBuilder) {
    this.chatClient = chatClientBuilder.build();
  }

  public List<String> writeBio(String profile) {

    String outline = this.chatClient.prompt().user( u -> u.text("""
         Create an outline for a professional biography that can be used in a speakers 
         directory for a conference. 
         """)).call().content();


    String bioDraft1 = this.chatClient.prompt().user( u -> u.text("""
        ...
        """).param("outline", outline).param("profile",profile)).call().content();

    String bioDraft2 = this.chatClient.prompt().user( u -> u.text(""" 
         Turn the biography below into a single paragraph suitable for use 
         in an important document. 
         --- 
        {bio}
        """).param("bio", bioDraft1)).call().content();


    String bioDraft3 = this.chatClient.prompt()
        .system("""
        ...
        .user(u -> u.text("""
            ...
            ---
            {bio}
            """).param("bio", bioDraft2)).call().content();


    String critique = this.chatClient.prompt()
        .system("""
        ...
        .user( u -> u.text("""
            ...
            """).param("bio",bioDraft3)).call().content();

    String finalBio = this.chatClient.prompt()
        .system("""
        ...
        .user(u -> u.text("""
            ...
            """).param("bio", bioDraft3).param("critique", critique).param("profile", profile)).call().content();
